...
function short(df::AbstractDataFrame; window=20, window2=50, min_lows=3, gain=0.1, neg=true, weights=vweights[])
...
function short(mrkts::AbstractDict; window=20, window2=50, sorted=true, rev=false, kwargs...)
    local df
...
function _short(mrkts::AbstractDict{String, DataFrame}; kwargs...)
    ...
    [(pair=k, short(p; kwargs...)...) for (k, p) in mrkts if size(p, 1) > maxw] |>
        DataFrame
end
