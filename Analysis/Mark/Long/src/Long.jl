...
function isbuyvol(df::AbstractDataFrame; kwargs...)
    isbuyvol(df.open, df.close, df.volume; kwargs...)
end
...
const rs_cache = Ref(
    hash(0) => (DataFrame(), (Vector{String}(), Matrix{Float64}(undef, 0, 0)))
)
...
"""
function relative_strength(
    pair::AbstractString, mrkts::AbstractDict; sorted=true, norm_roc=false
)
...
@views function pair_matrix(mrkts::AbstractDict)
    ...
    (
        catmarkets([copy(mrkts[p].high[(end - min_len + 1):end]) for p in order]),
        catmarkets([copy(mrkts[p].volume[(end - min_len + 1):end]) for p in order]),
        order,
    )
end
...
@views function stage2(
    ...
    is_trending = all((ma200[2:end] .> ma200[1:(end - 1)]))
    # 4
    ...
    is_uptrend_rs = all((slopeangle(rs_ts[(end - 84 - 1):end]; n=42) .> 0))
    rel_str = pair_rs > 0.8 && is_uptrend_rs
    # 8
    days50 = close[(end - 50 + 1):end]
    μ50 = mean(days50)
    outofbase = all((slopeangle(days50; n=baseσ)[(baseσ + 1):end] .< base°))
...
function _stage2(mrkts::AbstractDict{String,DataFrame}; kwargs...)
    ...
    data = [
        (pair=k, stage2(k, p.high, p.low, p.close, mrkts; kwargs...)...) for
        (k, p) in mrkts if size(p, 1) > 365
    ]
    if isempty(data)
        DataFrame(:pair => [], :score => [], (k => [] for k in keys(s2_weights[]))...)
    else
        DataFrame(data)
    end
end
...
const cweights = Ref((ft=0.15, up=0.05, bvol=0.25, tball=0.3, mvp=0.25))
...
function _longs(mrkts::AbstractDict{String,DataFrame}; kwargs...)
    ...
    DataFrame([(pair=k, long(p; kwargs...)...) for (k, p) in mrkts if size(p, 1) > maxw])
end
