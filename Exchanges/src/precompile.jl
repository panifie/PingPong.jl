...
@preset let
    ...
    @precomp let
        ...
        timestamp(e)
        check_timeout(e)
        tickers(e, qc; min_vol=1e4, verbose=false)
        tickers(e, qc; min_vol=1e4, with_margin=true, verbose=false)
        tickers(e, qc; min_vol=1e4, with_leverage=:yes, verbose=false)
        tickers(e, qc; min_vol=1e4, with_leverage=:only, verbose=false)
