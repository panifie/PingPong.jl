...
@kwdef struct SafeLock <: AbstractLock
    lock = ReentrantLock()
end

# Lock tracking data structures
const lock_graph = Dict{Task, Set{Task}}()  # Task -> Set of tasks it waits for
const held_locks = Dict{Task, Dict{SafeLock, Int}}()  # Task -> Locks it holds and their lock counts
const waiting_for_lock = Dict{Task, SafeLock}()  # Task -> The lock it is waiting for
const lock_to_task = Dict{SafeLock, Task}()  # SafeLock -> Task that holds it
...
# Function to acquire a SafeLock
function lock(l::SafeLock)
    ...
    if !haskey(held_locks, current)
        held_locks[current] = Dict{SafeLock, Int}()
    end
...
# Function to release a SafeLock
function unlock(l::SafeLock)
    current = current_task()
...
# Function to try to acquire a SafeLock (non-blocking)
function trylock(l::SafeLock)
    current = current_task()
...
function islocked(l::SafeLock)
    islocked(l.lock)
end

function getproperty(l::SafeLock, k::Symbol)
    l = getfield(l, :lock)
    if k == :lock
        l
    else
