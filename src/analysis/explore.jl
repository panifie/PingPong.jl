...
function find_bottomed(pairs::AbstractVector{PairData}; bb_thresh=0.05, up_thresh=0.05, n=12, mn=1., mx=90.)
    bottomed = Dict()
    for p in pairs
...
find_peaked(pairs::AbstractDict{String, PairData}; kwargs...) = find_peaked(collect(values(pairs)); kwargs...)

@doc "Find peaked shorts."
function find_peaked(pairs::AbstractVector{PairData}; bb_thresh=-0.05, up_thresh=0.05, n=12, mn=-0.90, mx=0)
    peaked = Dict()
    for p in pairs
